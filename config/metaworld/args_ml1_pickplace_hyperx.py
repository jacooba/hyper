...
def get_args(rest_args):
    ...
    parser.add_argument('--env_name', default='metaworld_ml1', help='environment to train on')
    parser.add_argument('--ml1_type', default='pick-place', help='push/pick-place/reach')
    parser.add_argument('--mw_version', type=int, default=1, help='version of meta-world')
    parser.add_argument('--sparse_metaworld_rewards', type=boolean_argument, default=False,
    ...
    parser.add_argument('--normalise_rew_targets', type=boolean_argument, default=True, help='divide reward targets by largest rew seen')
    ...
    parser.add_argument('--state_decoder_layers', nargs='+', type=int, default=[128, 64, 32])
    ...
    parser.add_argument('--intrinsic_rew_type', type=str, default='random_network_distillation')

    # what to compute the reward bonus for
    parser.add_argument('--intrinsic_rew_for_hyperstate', type=boolean_argument, default=True,
                        help='reward bonus on (s, b)')
    parser.add_argument('--intrinsic_rew_for_state', type=boolean_argument, default=False, help='reward bonus only on (s)')
    parser.add_argument('--intrinsic_rew_for_belief', type=boolean_argument, default=False, help='reward bonus only on (b)')
    parser.add_argument('--intrinsic_rew_include_actions', type=boolean_argument, default=False,
                        help='also reward novel actions (rew bonus for (s,b,a))')
    # whether or not to normalise intrinsic rewards
    parser.add_argument('--intrinsic_rew_normalise_rewards', type=boolean_argument, default=True)
    parser.add_argument('--intrinsic_rew_clip_rewards', type=float, default=10.)
    parser.add_argument('--intrinsic_rew_normalise_rew_sum', type=boolean_argument, default=False, help='normalise the sum of intrinsic and extrinsic reward')
    # weights for the rewards bonuses
    parser.add_argument('--rnd_weight_hyperstate', type=float, default=1.0, help='weight of additional reward / value')
    parser.add_argument('--rnd_weight_state', type=float, default=1.0, help='weight of additional reward / value')
    parser.add_argument('--rnd_weight_belief', type=float, default=1.0, help='weight of additional reward / value')

    # anneal the weight of the bonuses over the course of training
    parser.add_argument('--intrinsic_rew_anneal_weight', type=boolean_argument, default=True)

    # VAE rew reconstruction loss as reward
    parser.add_argument('--intrinsic_rew_for_vae_loss', type=boolean_argument, default=True)
    parser.add_argument('--intrinsic_weight_vae_loss', type=float, default=1.0, help='weight of additional reward')
    ...
    parser.add_argument('--layers_rnd_predictor', nargs='+', type=int, default=[256, 256])
    parser.add_argument('--rnd_use_orthogonal_init', type=boolean_argument, default=False,
