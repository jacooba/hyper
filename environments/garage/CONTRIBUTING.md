...
* Add convenience imports in `__init__.py` of a package for shallow first-level repetitive imports, but not for subpackages, even if that subpackage is defined in a single `.py` file.
    ...
    For instance, if an import line reads `from environments.garage.foo.bar import Bar` then you should add `from environments.garage.foo.bar import Bar` to `garage/foo/__init__.py` so that users may instead write `from environments.garage.foo import Bar`. However, if an import line reads `from environments.garage.foo.bar.stuff import Baz`, *do not* add `from environments.garage.foo.bar.stuff import Baz` to `garage/foo/__init__.py`, because that obscures the `stuff` subpackage.
    ...
    *Do*
    ...
    ```python
    """Foo package."""
    from environments.garage.foo.bar import Bar
    ```
    `garage/barp/bux.py`:
    ```python
    """Bux tools for barps."""
    from environments.garage.foo import Bar
    from environments.garage.foo.stuff import Baz
    ```
    ...
    *Don't*
    ...
    """Foo package."""
    from environments.garage.foo.bar import Bar
    from environments.garage.foo.bar.stuff import Baz
    ```
    `garage/barp/bux.py`:
    ```python
    """Bux tools for barps."""
    from environments.garage.foo import Bar
    from environments.garage.foo import Baz
    ```
* Imports within the same package should be absolute, to avoid creating circular dependencies due to convenience imports in `__init__.py`
    ...
    *Do*
    ...
    `garage/foo/bar.py`
    ```python
    from environments.garage.foo.baz import Baz
    ...
    *Don't*
    ...
    `garage/foo/bar.py`
    ```python
    from environments.garage.foo import Baz  # this could lead to a circular import, if Baz is imported in garage/foo/__init__.py
